...
class PrenotazioneController {
    ...
    static async get (req,res) {
        ...
        if ( req.accepts("html") ) {
            return res.render("viewsede",result);
        } else {
            return res.json(result);
        }
    }
    ...
    static async bakeCreationUpdateData (req,res) {
        try {
            ...
            let sede=[];
            sede=await Sede.lista(0,luogo,true);
            logger.debug("bakeCreationData: sedi trovate:",sede);
            return res.render("creaprenotazione",{listaPersone: persone,listaSede: sede, current:ep}) ;
        } catch (err) {
